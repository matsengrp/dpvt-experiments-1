...
rule create_fasta_with_cleaned_names:
    ...
    shell:
        ...
        ss2=$(echo "$seq2" | sed "s/>//g" | tr -d '[:space:]')
        initialstring="($ss1,$ss2)1;"
...
rule create_mat_protobuf:
    ...
    shell:
        """
        usher -t {input.nh_file} -v {input.vcf_file} -o {base_filename}_with_rootseq.pb
        matUtils extract -i {base_filename}_with_rootseq.pb -k "unambiguous_root_seq:1" --prune -o {base_filename}.pb
        """
