...
def evaluate_individual_trees(
    ...
    for i in range(len(test_data)):
        ...
        traversal = test_data.traversal[i:i+1]  # Keep batch dimension but select single tree
        mutations = test_data.mutations[i:i+1]  # Keep batch dimension but select single tree
        ...
        with torch.no_grad():
            ...
            logits = torch.stack([model.forward_on_traversal(t, m) for (t, m) in zip(traversal, mutations)]).squeeze(0)
        ...
        auroc_value = auroc(masked_probs, masked_labels.int()) if len(masked_labels) > 0 else float('nan')
        
        # Calculate accuracy
        predictions = (masked_probs > 0.5).float()
        accuracy = (predictions == masked_labels).float().mean().item() if len(masked_labels) > 0 else float('nan')
        ...
        avg_prob_pos = masked_probs[masked_labels == 1].mean().item() if (masked_labels == 1).sum() > 0 else float('nan')
        avg_prob_neg = masked_probs[masked_labels == 0].mean().item() if (masked_labels == 0).sum() > 0 else float('nan')
...
def initial_plots(results_df, plt_file):
        # Create summary plots
        fig, axes = plt.subplots(2, 2, figsize=(12, 10))
        
        # AUROC distribution
        axes[0, 0].hist(results_df['auroc'].dropna(), bins=20)
        axes[0, 0].set_title('AUROC Distribution')
        axes[0, 0].set_xlabel('AUROC')
        axes[0, 0].set_ylabel('Count')
        
        # Accuracy distribution
        axes[0, 1].hist(results_df['accuracy'].dropna(), bins=20)
        axes[0, 1].set_title('Accuracy Distribution')
        axes[0, 1].set_xlabel('Accuracy')
        axes[0, 1].set_ylabel('Count')
        
        # F1 distribution
        axes[1, 0].hist(results_df['f1'].dropna(), bins=20)
        axes[1, 0].set_title('F1 Score Distribution')
        axes[1, 0].set_xlabel('F1 Score')
        axes[1, 0].set_ylabel('Count')
        
        # Number of edges vs performance
        axes[1, 1].scatter(results_df['num_edges'], results_df['auroc'])
        axes[1, 1].set_title('Tree Size vs. AUROC')
