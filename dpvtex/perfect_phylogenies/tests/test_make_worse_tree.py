...
def test_make_worse_tree_basic():
    """Test that make_worse_tree produces a tree with higher parsimony score."""
    ...
    worse_tree = make_worse_tree(tree, depth=2, max_attempts=100)
...
def test_make_worse_tree_preserves_leaves():
    """Test that make_worse_tree preserves leaf sequences."""
    tree = Tree("(((((A,B),C),D),E),F);")
    sequences = ["AAAA", "AAAC", "AACC", "ACCC", "CCCC", "GGGG"]
    for node, seq in zip(tree.iter_leaves(), sequences):
        node.add_feature("sequence", seq)
...
def test_make_worse_tree_invalid_depth():
    """Test that make_worse_tree returns None for depth too large for tree."""
    ...
    worse_tree = make_worse_tree(tree, depth=10, max_attempts=10)
    assert worse_tree is None, "Should return None for depth larger than tree"


def test_make_worse_tree_no_improvement_found():
    """Test that make_worse_tree returns None if no worse tree is found."""
    ...
    worse_tree = make_worse_tree(tree, depth=1, max_attempts=5)
...
def test_make_worse_tree_different_depths():
    """Test make_worse_tree with various depth values."""
    ...
    for depth in [1, 2, 3]:
        worse_tree = make_worse_tree(tree, depth=depth, max_attempts=50)
        if worse_tree is not None:
            assert (
                parsimony_score(worse_tree) >= original_score
...
def test_make_worse_tree_internal_sequences():
...
def test_make_worse_tree_maintains_tree_structure():
    ...
    worse_tree = make_worse_tree(tree, depth=2, max_attempts=50)
...
def test_make_worse_tree_with_longer_sequences():
...
def test_make_worse_tree_with_8_leaves():
    ...
    for depth in [1, 2, 3]:
        worse_tree = make_worse_tree(tree, depth=depth, max_attempts=100)
        if worse_tree is not None:
...
def test_make_worse_tree_edge_cases():
    ...
    worse_tree = make_worse_tree(tree, depth=1, max_attempts=100)
    if worse_tree is not None:
    ...
    try:
        result = make_worse_tree(tree2, depth=0, max_attempts=10)
...
if __name__ == "__main__":
    ...
    test_make_worse_tree_basic()
    test_make_worse_tree_preserves_leaves()
    test_make_worse_tree_invalid_depth()
    test_make_worse_tree_no_improvement_found()
    test_make_worse_tree_different_depths()
    test_make_worse_tree_internal_sequences()
    test_make_worse_tree_maintains_tree_structure()
    test_make_worse_tree_with_longer_sequences()
    test_make_worse_tree_with_8_leaves()
    test_make_worse_tree_edge_cases()
