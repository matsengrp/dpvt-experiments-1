...
output_data = config.get(
    "output_data", config.get("larch_output")
)  # Support both names
...
balance_by_median_num_MP_trees = config.get(
    "balance_by_median_num_MP_trees", True
)  # Default to True - balance trees per alignment
larch_timeout = config.get(
    "larch_timeout", 1800
)  # Default timeout: 1800 seconds (30 minutes)

# Support both single edge_distribution (string) and multiple (list)
edge_distributions = config.get(
    "edge_distributions", config.get("edge_distribution", "constant")
)
...
spr_target_non_mp_proportion = config.get(
    "spr_target_non_mp_proportion", 0.1
)  # Target non-MP edge proportion
max_spr_attempts = config.get(
    "max_spr_attempts", 100
)  # Max SPR attempts before stopping

# Subtree replacement parameters
subtree_max_attempts = config.get(
    "subtree_max_attempts", 100
)  # Max attempts for subtree replacement
subtree_target_non_mp_proportion = config.get(
    "subtree_target_non_mp_proportion", 1 / 6
)  # Target non-MP edge proportion
_subtree_depth_raw = config.get("subtree_depth", None)
subtree_depth = (
    None
    if _subtree_depth_raw in (None, "None", "null", "")
    else int(_subtree_depth_raw)
)
    ...
    run:
        ...
        try:
            ...
            if result.returncode == 124:  # timeout command returns 124 on timeout
                ...
                logger.log(
                    "LARCH_TIMEOUT",
                    f"Larch timed out after {params.timeout} seconds for alignment: {params.subdir}",
                )
                print(
                    f"WARNING: Larch timed out for {params.subdir}. Excluding from pipeline.",
                    file=sys.stderr,
                )
                ...
                logger.log(
                    "LARCH_ERROR",
                    f"Larch failed with return code {result.returncode} for alignment: {params.subdir}",
                )
                logger.log("LARCH_ERROR", f"STDERR: {result.stderr}")
                print(
                    f"WARNING: Larch failed for {params.subdir}. Excluding from pipeline.",
                    file=sys.stderr,
                )
            ...
            print(
                f"WARNING: Unexpected error for {params.subdir}: {str(e)}",
                file=sys.stderr,
            )
            # Create empty output to allow pipeline to continue
            with open(output.pb, "w") as f:
...
rule extract_dpvt_data:
    ...
    output:
        ...
        num_children_file=input_data
        + "/{subdir}/num_children_dag_trees{edge_suffix}"
        + dup_sites_suffix
        + ".csv",
...
rule aggregate_training_data:
    input:
        ...
        size_stats=expand(
            input_data + "/{subdir}/size_stats" + dup_sites_suffix + ".csv",
            subdir=get_subdirs(input_data),
        ),
    output:
        data_props=input_data
        + "/data_properties_"
        + dataset_name
        + "{edge_suffix}"
        + dup_sites_suffix
        + ".csv",
        dpvt_data=output_data
        + "/"
        + dataset_name
        + "{edge_suffix}"
        + dup_sites_suffix
    ...
    run:
        ...
        aggregate_data(
            data_dir=input_data,
            data_props_file=output.data_props,
            dpvt_train_data=output.dpvt_data,
            edge_distribution=edge_dist,
            dpvt_test_data=None,
