...
use the script `create_alisim_alignments.sh`, which uses IQ-TREE's alisim to
simulate alignments
([http://www.iqtree.org/doc/AliSim](http://www.iqtree.org/doc/AliSim)). At the
beginning of this script you need so specify the number of alignments, number of
sequences per alignment, and number of sites per alignment. The script can
create alignments for varying numbers of sequences and sites and saves each
dataset of alignments into its own directory. It also generates config files
that can be used in the next step for running the Snakemake workflow for
generating training and testing data for `dpvt`.
...
`dpvt` in `/path/to/dpvt/training/data`. The following steps are executed by
this script:
...
-   perturbs the sampled trees by replacing a subtree with depth `tree_depth/3`
    by a random tree
...
-   Called from `generate_dpvt_input.snakefile`:
    ...
    -   `extract_data_from_hdag.py`: Reads hDAGs produced by larch and extracts
        ...
        -   use `make_worse_tree()` function from
            ...
            edges by replacing a subtree by a random tree
