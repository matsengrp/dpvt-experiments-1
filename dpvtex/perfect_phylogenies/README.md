...
To generate perfect phylogeny datasets as training and testing sets for `dpvt`,
run the script `dpvtex/perfect_phylogenies/examples/make_datasets.py` and hand
over command line arguments for the desired number of trees, leaves, and whether
SPR moves should be used to create non-MP edges, i.e.:

 `python make_datasets.py <num_leaves> <num_trees> <spr>`

where `spr` is `True` (i.e. SPRs are used to generate non-MP edges) or `False`
