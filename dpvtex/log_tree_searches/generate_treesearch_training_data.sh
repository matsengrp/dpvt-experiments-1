...
CONFIG=config_pandit_treesearch_training_nj.yaml
START_TYPE=nj
OUTPUT_DIR=../../data/treesearch_training_pandit
MERGED_PICKLE=../../data/pandit_treesearch_nj_training.p
NICKNAMES_FILE=../../train/my_treesearch_nicknames.json
NICKNAME_KEY=pandit_treesearch_nj_training
...
pixi run python - "$NICKNAMES_FILE" "$NICKNAME_KEY" "$(basename "$MERGED_PICKLE")" <<'PYEOF'
import json, sys
path = sys.argv[1]
key = sys.argv[2]
value = sys.argv[3]
